...
`swift-pwa` is a Swift-native thin-client PWA wrapper around system webviews — a Tauri/Wails analogue for the Swift world. One Swift package, one JS API (`__SWIFT_PWA__.invoke()` / `subscribe()`), one CLI (`swift-pwa`) that produces `.app`, `.ipa`, `.AppImage`, and Windows portable / MSIX from the same source. Status: last tagged release is v0.6.3 (all five platforms first-class). v0.6.3 adds **runtime content packs**: `ctx.serveDirectory(_:at:)` (serve a writable dir on the bundle origin under an app-chosen prefix, range-aware on every backend) + `fs.extractZip`/`fs.listZip` (native path-to-path extraction — ZIPFoundation on macOS/iOS/Linux, `tar.exe` on Windows, `java.util.zip`-via-JNI on Android — guards: traversal/symlink/zip-bomb), device-verified on Android. The rest of the v0.6.x line is a CLI ease-of-use series: `init` scaffolds a GitHub Actions release workflow (+ a `generate-ci` subcommand; the CLI version is pinned once via a `SWIFT_PWA_CLI_VERSION` env var), one `pwa.json` icon PNG becomes the iOS + Android app icon (not just macOS), `swift-pwa doctor` checks per-target toolchain prerequisites *and* flags a generated `App.swift` that lags the CLI (the scaffold is version-stamped), `build --target macos --notarize` automates notarization, `swift-pwa dev` runs a built-in live-reload server (POSIX), a built-in `AppPlugin` exposes `app.quit`/`app.name`/`app.version`, `pwa.json`'s `build.prebuild` runs a codegen/asset command before bundling, and `swift-pwa self-update` updates the CLI in place (atomic rename, dodging the macOS stale-codesign-cache SIGKILL). `executable_name` is auto-discovered from the package via `swift package describe` (rarely needed).
