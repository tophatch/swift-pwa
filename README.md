...
> **Status:** [`v0.8.12`](https://github.com/tophatch/swift-pwa/releases/tag/v0.8.12) is the current release — macOS 15+, iOS 18+, Linux (GTK3 / GTK4), Windows 11 (WebView2), and Android (API 28+) are all first-class, with **built-in on-device AI on every platform**: the OS platform model (Apple Foundation Models, Android Gemini Nano, Windows Phi Silica) *and* portable GPU/CPU-accelerated llama.cpp (Apple Metal, Linux + Windows x64 Vulkan, Windows arm64). **The on-device ONNX Runtime tier now spans three consumers of `ai.generateImage`/`ai.vision.*`**: promptable image *segmentation* (`MobileSAMBackend`), image *editing* / inpainting (`LaMaBackend`), and *text→image* generation (`StableDiffusionBackend`) — with **two verified models**: SD-Turbo and **LCM_Dreamshaper** (the OpenRAIL-M / commercially-usable few-step default), both matched to a diffusers reference. `ai.generateImage` is a purpose-agnostic op (text→image / img2img / inpaint, chosen by which fields you send). New in **0.8.12**: a **config-driven remote image provider** — `RESTImageProvider` adapts to an arbitrary cloud image API from a **descriptor** (endpoint + request template + response JSONPath, one-shot or async submit→poll, JSON or multipart), instead of a hand-written Swift conformance per service; the descriptor travels in the call, so a web app plugs in a new API with no rebuild. Ships presets for **Imagen, Gemini image ("nano banana"), OpenAI (generations + edits), and Qwen/DashScope** — verified live against the real APIs (Gemini/Imagen/Qwen-async), demoed as a picker option in `Examples/CritterFacts`. New in **0.8.11**: the **runtime workflow surface is now provider-agnostic** — `ai.run` / `ai.describeInputs` work beyond ComfyUI, routed by a `provider` id: **Imagen** (a fixed-schema cloud provider) and **on-device** Stable Diffusion / LaMa (via a generic `AIBackendWorkflowProvider` that adapts *any* `AIBackend`) answer the same `describeInputs` → controls → `run` loop, no graph and no connection required, so one JS page (`web/workflow.html`, now with a provider picker) drives all three. New in **0.8.10**: a **runtime, JS-reachable workflow surface** — `ai.run` / `ai.describeInputs` (`AIWorkflowPlugin`) lets a web app import a ComfyUI graph, introspect its overridable inputs, and run it with **live per-step progress** (a cross-platform `NetworkClient.openWebSocket`, including an Android OkHttp `net.ws` RPC), **cancel**, and **job recovery** (re-attach to an in-flight job by id after a dropped stream) — the graph *and* the connection travel in each call, so there's no rebuild per workflow or per box (device-verified on a Galaxy Tab S10+; demoed in `Examples/CritterFacts`' `web/workflow.html`). New in **0.8.9**: the **ComfyUI provider runs *any* workflow you import** — export a graph as "Save (API Format)", then `runWorkflow(inputs:)` executes it (arbitrary named inputs, image upload for img2img/edit, one input fanning out across nodes), `inspectWorkflow` lists its overridable inputs (types/ranges/options, via the instance's `/object_info`) so a UI builds controls automatically, and `ComfyWorkflowProvider` wraps one as a turnkey `ai.*` model in the switcher — so the app owns importing workflows and the framework runs them, no per-architecture code (verified live against Qwen-Image txt2img, an edit, and an upscale). 0.8.8 added **remote/cloud image generators** — a first-class **`net.*`** native-HTTP plugin (CORS-free, reaches LAN appliances) plus a `SwiftPWARemoteAI` tier with **Google Imagen** and **local-network ComfyUI** providers, so a cloud API or a box on your LAN drops into the same `ai.generateImage` switcher next to on-device models (a third API is a small protocol conformance); and a **`secrets.*` secure-storage plugin** — **Keychain / Android Keystore / Windows DPAPI / Linux libsecret** behind one JS+Swift API — so a cloud API key lives in the OS keychain (never `localStorage`/`pwa.json`) and a model can advertise **`needsSetup` until a key is entered at runtime**, then flip to `ready`. (0.8.7 added the runtime **model/backend switcher** — `ai.info().models` + `request.model` + the `MultiModelImageBackend` router that frees the previous model on switch — plus byte-level Android model-download progress.) All ride a checksum-pinned downloadable-model tier and the `ai.local_onnx_runtime` `pwa.json` opt-in; the optional **desktop GPU tier** (`ai.onnx_gpu` — Windows DirectML / Linux CUDA, auto-detect + CPU fallback) accelerates them. Tap-to-segment, tap-to-erase, and a prompt-to-image **model switcher** demoed in `Examples/CritterFacts`. See the [feature matrix](#feature-matrix), the per-platform [setup docs](docs/), and [`CHANGELOG.md`](CHANGELOG.md).
...
- **v0.8.4** (released) — **completes `ai.generateImage` inpainting on all five platforms** with the **Android** image codec (BitmapFactory decode + `Bitmap.compress` encode over the Kotlin RPC — Apple/desktop already shipped in 0.8.3), device-verified on a Galaxy Tab S10+. Also **fixes large-photo handling**: the backend now decodes + composites at a bounded working resolution (`LaMaModelSpec.maxWorkingSide`, default 2048) instead of the source's full resolution, so a 24-megapixel phone photo no longer OOMs (it did nothing on-device before).
- **v0.8.3** (released) — **on-device image editing (`ai.generateImage` inpainting) + a purpose-agnostic image op.** `ai.generateImage` (contract-only since 0.7.0) is generalized so the *operation* is chosen by which fields are present — `prompt`→text→image, `prompt`+`image`→img2img, `image`+`mask`→inpaint — rather than a command per mode; the model and purpose are the adopter's choice, the shipped backends being examples, not doctrine. The first backend, **`LaMaBackend`** (`SwiftPWAImageEdit`), inpaints via a LaMa ONNX model on the shared ONNX Runtime tier (extracted into **`SwiftPWAONNX`** so backends reuse the same `OrtModelSession` + desktop GPU providers). It pairs with segmentation — a SAM mask *is* the edit mask — for **tap-to-erase** (demoed in `Examples/CritterFacts`). Verified against the real big-lama fp32 weights on Apple/CPU + Linux/CPU; the `lama-vendor` weights release is checksum-pinned + downloadable (`ai.ensureModel`). Image decode/encode is CoreGraphics (Apple) / stb_image (Linux/Windows). Also fixes an Apple `ai.vision.*` preprocessing bug (the encoder was fed a vertically-flipped image). See [docs/proposals/image-generation-editing.md](docs/proposals/image-generation-editing.md).
- **v0.8.2** (released) — **desktop GPU execution providers for `ai.vision.*`.** The Linux/Windows `MobileSAMBackend` shipped CPU-only in 0.8.1; a build-time opt-in — the sibling boolean **`ai.onnx_gpu: true`** in `pwa.json` — now ships a GPU-capable ONNX Runtime: **Windows DirectML** (cross-vendor — any DX12 GPU — and in-box on Win10+) and **Linux CUDA 12** (NVIDIA; CUDA runtime + cuDNN expected on the target, not bundled). Runtime is **auto-detect with transparent CPU fallback** — the GPU EP is appended before the CPU EP at session creation, and if it can't initialize it logs once and retries on CPU, so a missing/unsupported GPU never breaks inference; `ai.vision.info` reports the active **`provider`** (`cuda`/`directml`/`cpu`). Verified on real hardware against the real weights: DirectML on a Radeon 780M (mask IoU `1.00000` vs CPU) and CUDA on an RTX 5080 (IoU `0.99997`, ~27× faster encode). There is no Vulkan-style one-artifact-all-vendors GPU layer for ONNX Runtime as there is for llama.cpp — see [docs/proposals/onnx-gpu-execution-providers.md](docs/proposals/onnx-gpu-execution-providers.md) for the research behind the vendor-specific split.
...
7. On-device segmentation (`ai.vision.*`) reached full cross-platform parity in **v0.8.1**, **v0.8.2** added the **GPU execution-provider desktop build** (`ai.onnx_gpu: true` — Windows DirectML / Linux CUDA, auto-detect + CPU fallback), and **v0.8.3**/**v0.8.4** added on-device image **editing** (`ai.generateImage` inpainting, `LaMaBackend`) as the ONNX tier's second consumer, on all five platforms — all above.
