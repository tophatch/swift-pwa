...
> **Status:** [`v0.8.6`](https://github.com/tophatch/swift-pwa/releases/tag/v0.8.6) is the current release — macOS 15+, iOS 18+, Linux (GTK3 / GTK4), Windows 11 (WebView2), and Android (API 28+) are all first-class, with **built-in on-device AI on every platform**: the OS platform model (Apple Foundation Models, Android Gemini Nano, Windows Phi Silica) *and* portable GPU/CPU-accelerated llama.cpp (Apple Metal, Linux + Windows x64 Vulkan, Windows arm64). **The on-device ONNX Runtime tier now spans three consumers of `ai.generateImage`/`ai.vision.*`**: promptable image *segmentation* (`MobileSAMBackend`), image *editing* / inpainting (`LaMaBackend`), and *text→image* generation (`StableDiffusionBackend`) — with **two verified models**: SD-Turbo and, new in 0.8.6, **LCM_Dreamshaper** (the OpenRAIL-M / commercially-usable few-step default), both matched to a diffusers reference. `ai.generateImage` is a purpose-agnostic op (text→image / img2img / inpaint, chosen by which fields you send). All ride a checksum-pinned downloadable-model tier and the `ai.local_onnx_runtime` `pwa.json` opt-in; the optional **desktop GPU tier** (`ai.onnx_gpu` — Windows DirectML / Linux CUDA, auto-detect + CPU fallback) accelerates them. Tap-to-segment, tap-to-erase, and prompt-to-image demoed in `Examples/CritterFacts`. See the [feature matrix](#feature-matrix), the per-platform [setup docs](docs/), and [`CHANGELOG.md`](CHANGELOG.md).
