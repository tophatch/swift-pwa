...
> **Status:** [`v0.8.12`](https://github.com/tophatch/swift-pwa/releases/tag/v0.8.12) is the current release. macOS 15+, iOS 18+, Linux (GTK3 / GTK4), Windows 11 (WebView2), and Android (API 28+) are all first-class — one Swift package, one JS API, one CLI, five platforms from the same source.
>
...
1. **Auto-updates.** The publishing side (`swift-pwa updater keygen` / `sign` / `manifest`, Ed25519, the manifest format) is solid; the runtime install path needs work:
   - **Verify + harden the desktop runtime updater.** The `UpdaterPlugin` runtime is marked **Untested** on all five desktop platforms (feature-matrix footnote 6) — only Android's `PackageInstaller` path is exercised end-to-end. What's unverified is each backend's `Updater.installAndRelaunch` (download → signature-verify → atomic swap → relaunch). Goal: drive a real check→download→install→relaunch on macOS / Windows / Linux (AppImage), flip those matrix cells to `Yes`, and add integration coverage. Unblocks the two below. Context: [docs/auto-updates.md](docs/auto-updates.md).
