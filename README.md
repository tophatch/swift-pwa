...
- **v0.5** (in flight on `main`) — **Android backend at desktop parity:** `SwiftPWAAndroid` Swift target, JNI shim, Kotlin / Gradle scaffold, `swift-pwa build --target android` with always-on `llvm-strip` (131 MB → 80 MB APK out of the box) plus an opt-in `--prune-android-runtime` flag (further 4 MB), and the full `Clipboard` / `Notifications` / `Dialog` / `BiometricAuth` / `Updater` (`PackageInstaller.Session`) plugin set behind a Swift→Kotlin RPC channel. CI runs the cross-compile + `assembleDebug` pipeline on every push. See [docs/android-setup.md](docs/android-setup.md) for the wiring and [docs/android-on-device-testing.md](docs/android-on-device-testing.md) for the on-device verification loop (Chrome DevTools Protocol over `adb forward` + a small Python helper). Still queued for the v0.5.x cycle: GTK4 tray, Android codesigning wiring, `swift-pwa init` Android boilerplate.
