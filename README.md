...
7. Android backend lands in v0.5.x with the full `Clipboard` / `Notifications` / `Dialog` / `BiometricAuth` / `Updater` plugin set behind a Swift→Kotlin RPC channel; **end-to-end verified on a Galaxy Tab S10+** (every plugin's RPC round-trip exercised via Chrome DevTools Protocol). See [docs/android-setup.md](docs/android-setup.md) for the wiring + boilerplate, and [docs/android-on-device-testing.md](docs/android-on-device-testing.md) for the on-device test loop.
...
10. Android signing goes through Gradle's `signingConfigs`. The bundler emits a `release` config when `pwa.json`'s `android.signing` is set or `--sign <keystore> --android-key-alias <alias>` is passed; passwords come from the `SWIFT_PWA_ANDROID_STORE_PASSWORD` / `SWIFT_PWA_ANDROID_KEY_PASSWORD` environment variables so `pwa.json` stays committable. See [docs/android-setup.md](docs/android-setup.md) §7 for the full wiring (including a CI base64-keystore pattern).
...
- **v0.5** (in flight on `main`) — **Android backend at desktop parity:** `SwiftPWAAndroid` Swift target, JNI shim, Kotlin / Gradle scaffold, `swift-pwa build --target android` with always-on `llvm-strip` (131 MB → 80 MB APK out of the box) plus an opt-in `--prune-android-runtime` flag (further 4 MB), and the full `Clipboard` / `Notifications` / `Dialog` / `BiometricAuth` / `Updater` (`PackageInstaller.Session`) plugin set behind a Swift→Kotlin RPC channel. CI runs the cross-compile + `assembleDebug` pipeline on every push. Release signing is wired through Gradle `signingConfigs` driven by `pwa.json`'s `android.signing` section (or `--sign / --android-key-alias` CLI overrides) with passwords sourced from environment variables; `swift-pwa init` lays down the JNI entry-point boilerplate (`Sources/<name>/AndroidEntry.swift`) pre-populated with the right `@_cdecl` mangled symbol for the chosen bundle id. See [docs/android-setup.md](docs/android-setup.md) for the wiring and [docs/android-on-device-testing.md](docs/android-on-device-testing.md) for the on-device verification loop (Chrome DevTools Protocol over `adb forward` + a small Python helper). Still queued for the v0.5.x cycle: GTK4 tray.
