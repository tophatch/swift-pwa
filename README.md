...
3. **On-device AI backends.** The `ai.*` plugin contract shipped (0.7) — `ai.info` / `ai.generate` / `ai.generateJSON` / streaming, plus multimodal **vision input** and **text→image generation** (`ai.generateImage`), schema-valid JSON on any backend — but no on-device backend is wired yet, so `ai.info` reports `available:false`. Next: the tier-1 platform built-ins (Apple Foundation Models, Android Gemini Nano, Windows Phi Silica), the portable Gemma fallback (MLX / MediaPipe / ONNX / llama.cpp), and text→image backends (Image Playground / Stable Diffusion), each in its own optional target like the zip backends. Context: [docs/ai-plugin.md](docs/ai-plugin.md).
4. **Delta updates** — ship binary diffs instead of full artifacts to cut update download size. Builds on (1); extend the publishing CLI to emit per-version patches and the runtime to apply them.
5. **Mandatory-update kill-switch (`min_supported_version`)** — let a manifest force-upgrade clients below a floor. Called out as a known gap in [docs/auto-updates.md](docs/auto-updates.md); also builds on (1).
