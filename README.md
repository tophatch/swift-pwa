...
1. **Auto-updates.** The publishing side (`swift-pwa updater keygen` / `sign` / `manifest`, Ed25519, the manifest format) is solid; the runtime install path needs work:
   ...
   - **Runtime auto-check polling.** `updater.check` is on-demand only; `pwa.json`'s `auto_check` / `check_interval_seconds` aren't yet honored at runtime. Wire an opt-in polling timer that emits an event when an update appears.
