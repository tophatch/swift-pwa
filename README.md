...
4. **On-device segmentation (`ai.vision.*`) — in progress.** The contract (`SegmentationBackend`, `VisionPlugin`, `ai.vision.info` / `openSession` / `segment` / `closeSession`, reserved `segmentAll(Stream)` / `ensureModel` / `benchmark`) has landed in `SwiftPWACore`. The v0.8 epic is the **ONNX Runtime tier** it needs (SAM-family models — MobileSAM ONNX encoder + decoder, ~4 MB/embedding session cache, `bounds` + row-major `rle` mask output) on Apple + Android; a packaging spike has verified both platforms end-to-end (`Scripts/vendor-onnxruntime-apple.sh` and `-android.sh` repackage Microsoft's official artifacts into an importable/linkable shape — neither works out of the box), Android confirmed **on an actual Galaxy Tab S10+** via `adb shell`. Still open: a real `MobileSAMBackend`, and publishing either vendored artifact as a release asset. Desktop backends (Linux/Windows) ship a `NoneBackend` in 0.8 (a documented parity exception, not an oversight); `segmentAll`/AMG, multi-point refine, and `ai.vision.benchmark` are fast-follow. Context: [docs/proposals/segmentation-plugin.md](docs/proposals/segmentation-plugin.md).
