...
4. **On-device segmentation (`ai.vision.*`) — in progress.** The contract (`SegmentationBackend`, `VisionPlugin`, `ai.vision.info` / `openSession` / `segment` / `closeSession`, reserved `segmentAll(Stream)` / `ensureModel` / `benchmark`) has landed in `SwiftPWACore`, and a real backend — **`MobileSAMBackend`** (Apple, `SwiftPWASegmentation`) — runs MobileSAM's encoder + decoder as two **ONNX Runtime** sessions, with independently-tested image preprocessing (SAM's resize/pad/normalize) and mask postprocessing (upsample + RLE). The encoder's I/O contract is verified against a real exported MobileSAM ONNX graph; the decoder's follows the same standard reference-SAM contract every MobileSAM port uses, proven end-to-end via structurally-matching fake-weight fixtures — **real decoder weights aren't verified yet, and model hosting is still an open decision.** The packaging side (`Scripts/vendor-onnxruntime-apple.sh` / `-android.sh`, repackaging Microsoft's official artifacts, which don't import/link out of the box) is verified on both Apple and Android — Android confirmed **on an actual Galaxy Tab S10+** via `adb shell` — with publish workflows ready but not yet run. Still open: real decoder-weight verification, model hosting/download, and the Android backend (Apple-only for now). Desktop backends (Linux/Windows) ship a `NoneBackend` in 0.8 (a documented parity exception, not an oversight); `segmentAll`/AMG, multi-point refine, and `ai.vision.benchmark` are fast-follow. Context: [docs/proposals/segmentation-plugin.md](docs/proposals/segmentation-plugin.md).
