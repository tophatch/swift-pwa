...
> **Status:** [`v0.9.12`](https://github.com/tophatch/swift-pwa/releases/tag/v0.9.12) is the current release. macOS 15+, iOS 18+, Linux (GTK3 / GTK4), Windows 11 (WebView2), and Android (API 28+) are all first-class — one Swift package, one JS API, one CLI, five platforms from the same source.
>
...
| App driver (`drive`/`mcp`)²²  |           Yes           |          Partial²²           |            Yes             |        Partial²²        |        Partial²²         |           —²³            |
...
22. `swift-pwa drive` / `swift-pwa mcp` are **development** tools, compiled into debug builds only (a release binary doesn't contain the driver at all). *Screenshots* work on all four desktop backends and on the iOS **Simulator** — each backend's own renderer snapshot, not the screen. *Synthetic input* (`click` / `type` / `scroll`) is macOS + GTK3 only: GTK4 removed public event synthesis, WebView2's `SendPointerInput` needs a composition controller swift-pwa doesn't create, and iOS has no public event-synthesis API. All report honestly via `drive info` and point you at `eval`. iOS is simulator-only — a device's loopback isn't the host's, so nothing can attach; `drive --simulator` builds, installs, and drives there (`eval`, `shot`, `windows`, `--route`). Windows is compile-verified but has not been driven against a live app. Guide: [docs/tutorials/testing-your-app.md](docs/tutorials/testing-your-app.md).
...
3. **One device surface — `permissions.*`, then `geo.*`.** A page's own `getUserMedia` / `navigator.geolocation` work only where something answers the webview's permission request, and until 0.10 nothing did on any backend. Linux now answers (both GTK backends, so capture *and* location work there for the first time) via a declaration the app makes once — see [docs/permissions.md](docs/permissions.md). Remaining: **Android** capture, which is the other real outage; a `pwa.json` declaration driving the manifest / plist entries; and a `geo.*` plugin, since macOS offers no public way to grant location through the web API. Context: [docs/proposals/permissions-bridge.md](docs/proposals/permissions-bridge.md).
4. **Platform audio (capture / playback).** Native microphone capture and audio playback / device routing behind a plugin — lower-latency and more capable than the WebView's `getUserMedia` / `MediaRecorder` once the permission gap above is closed everywhere (including feeding recordings to the `ai.*` audio input). Pairs with the on-device AI audio backend.
