...
| `UpdaterPlugin` (runtime)     |          Yes⁶           | Untested⁶                    | Untested⁶                  | Untested⁶               | Untested⁶                | Yes (PackageInstaller)   |
...
6. **macOS** is verified end-to-end (a real signed check→download→verify→`ditto`-swap→relaunch cycle, ad-hoc and codesigned) and Android's `PackageInstaller` path is exercised; iOS / Linux / Windows runtime backends are unit-tested but their per-OS install hand-off is preview until [docs/manual-test-cases.md](docs/manual-test-cases.md) is walked on each. The publishing CLI is fully tested. Full breakdown: [docs/auto-updates.md](docs/auto-updates.md).
...
1. **Auto-updates.** The publishing side (`swift-pwa updater keygen` / `sign` / `manifest`, Ed25519, the manifest format) is solid; the runtime install path needs work:
   - **Verify + harden the desktop runtime updater.** **macOS is now verified** end-to-end (a real check→download→verify→`ditto`-swap→relaunch cycle, ad-hoc + codesigned, which hardened three staging/cleanup gaps). Still **Untested**: **Windows** (portable + MSIX) and **Linux** (AppImage) — drive the same real cycle on each, flip those feature-matrix cells to `Yes`, add integration coverage. (iOS's `itms-services://` path needs an enterprise cert.) Unblocks the two below. Context: [docs/auto-updates.md](docs/auto-updates.md).
   ...
   - **Mandatory-update kill-switch (`min_supported_version`)** — let a manifest force-upgrade clients below a floor. A known gap in [docs/auto-updates.md](docs/auto-updates.md).
