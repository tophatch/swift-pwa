...
> **Status:** [`v0.8.3`](https://github.com/tophatch/swift-pwa/releases/tag/v0.8.3) is the current release — macOS 15+, iOS 18+, Linux (GTK3 / GTK4), Windows 11 (WebView2), and Android (API 28+) are all first-class, with **built-in on-device AI on every platform**: the OS platform model (Apple Foundation Models, Android Gemini Nano, Windows Phi Silica) *and* portable GPU/CPU-accelerated llama.cpp (Apple Metal, Linux + Windows x64 Vulkan, Windows arm64). **The on-device ONNX Runtime tier now spans two consumers**: promptable image *segmentation* via `ai.vision.*` (`MobileSAMBackend`, all five platforms) and — new in **0.8.3** — on-device image *editing* via `ai.generateImage` (`LaMaBackend` inpainting; Apple + Linux/Windows, Android codec pending), which pairs with segmentation for a tap-to-erase flow. `ai.generateImage` is now a purpose-agnostic op (text→image / img2img / inpaint, chosen by which fields you send). Both ride a checksum-pinned downloadable-model tier and the `ai.local_onnx_runtime` `pwa.json` opt-in; 0.8.2's optional **desktop GPU tier** (`ai.onnx_gpu` — Windows DirectML / Linux CUDA, auto-detect + CPU fallback) accelerates both. Tap-to-segment and tap-to-erase demoed in `Examples/CritterFacts`. See the [feature matrix](#feature-matrix), the per-platform [setup docs](docs/), and [`CHANGELOG.md`](CHANGELOG.md).
...
7. On-device segmentation (`ai.vision.*`) reached full cross-platform parity in **v0.8.1**, **v0.8.2** added the **GPU execution-provider desktop build** (`ai.onnx_gpu: true` — Windows DirectML / Linux CUDA, auto-detect + CPU fallback), and **v0.8.3** added on-device image **editing** (`ai.generateImage` inpainting, `LaMaBackend`) as the ONNX tier's second consumer — all above.
