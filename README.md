...
> **Status:** [`v0.8.0`](https://github.com/tophatch/swift-pwa/releases/tag/v0.8.0) is the current release — macOS 15+, iOS 18+, Linux (GTK3 / GTK4), Windows 11 (WebView2), and Android (API 28+) are all first-class, with **built-in on-device AI on every platform**: the OS platform model (Apple Foundation Models, Android Gemini Nano, Windows Phi Silica) *and* portable GPU/CPU-accelerated llama.cpp (Apple Metal, Linux + Windows x64 Vulkan, Windows arm64). **0.8.0 adds the on-device ONNX Runtime tier: promptable image segmentation via `ai.vision.*` (`MobileSAMBackend`, all platforms — Apple + Android in 0.8.0, Linux + Windows since), with a checksum-pinned downloadable-model tier (`ai.vision.ensureModel`) and a `pwa.json` opt-in (`ai.local_onnx_runtime`) that stages everything into the build automatically — tap-to-segment demoed in `Examples/CritterFacts`.** See the [feature matrix](#feature-matrix), the per-platform [setup docs](docs/), and [`CHANGELOG.md`](CHANGELOG.md).
...
On-device segmentation (`ai.vision.*`) shipped in **v0.8.0** (above); since then `segmentAll`/AMG (automatic "segment everything" mask generation), `ai.vision.benchmark` (device-capability timing), and the Linux + Windows desktop backends (CPU ONNX Runtime) all landed on `MobileSAMBackend`, completing cross-platform parity. A GPU execution-provider desktop build is the remaining fast-follow.
