...
- **Windows Hello reliably succeeds only under MSIX identity.** `UserConsentVerifier::RequestVerificationAsync` may report `DeviceNotPresent` from an unpackaged portable EXE even on hosts with Windows Hello configured. The raw status is surfaced in `BiometricAvailability.reason` — apps that need biometric prompts on every install should ship MSIX. See [docs/windows-setup.md](docs/windows-setup.md).
