...
4. **On-device segmentation (`ai.vision.*`).** A `SegmentationBackend` in its own optional target (`SwiftPWASegmentation`), installed via `VisionPlugin`, under the `ai.vision.*` namespace. The v0.8 epic is the **ONNX Runtime tier**: `ai.vision.info` / `openSession` / `segment` / `closeSession` / `ensureModel` on Apple + Android (SAM-family models — MobileSAM ONNX encoder + decoder, ~4 MB/embedding session cache, `bounds` + row-major `rle` mask output). Desktop backends (Linux/Windows) ship a `NoneBackend` in 0.8 (a documented parity exception, not an oversight); `segmentAll`/AMG, multi-point refine, and `ai.vision.benchmark` are fast-follow. Context: [docs/proposals/segmentation-plugin.md](docs/proposals/segmentation-plugin.md) (maintainer decision section).
