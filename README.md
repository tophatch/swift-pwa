...
3. **On-device AI backends.** The `ai.*` plugin contract shipped (0.7) — `ai.info` / `ai.generate` / `ai.generateJSON` / streaming, plus multimodal **vision + audio input**, **text→image generation** (`ai.generateImage`), and **text→audio / TTS** (`ai.generateAudio`), schema-valid JSON on any backend. The platform built-ins are now implemented on **every** OS that ships one — **Apple Foundation Models** (`SwiftPWAFoundationModels`) on Apple, **Android Gemini Nano** (`GeminiNanoBackend`, via ML Kit GenAI / AICore; `ai.gemini_nano`) on Android, and **Windows Phi Silica** (`PhiSilicaBackend`, via the Windows AI APIs in the Windows App SDK; `ai.phi_silica`) on Windows — alongside the portable **llama.cpp** backend (`SwiftPWALlama` — runs a GGUF model, GBNF-constrained JSON; `ai.local_llama`) on **Apple (Metal), Linux x86_64 (Vulkan GPU), and Windows (x64 Vulkan GPU + arm64 CPU)**. (Phi Silica needs an MSIX-packaged build + a Microsoft Limited-Access-Feature token — both Windows-platform requirements, wired via `PhiSilicaBackend(unlockToken:)`; on arm64 Copilot+ devices the portable llama.cpp backend — CPU-only for now, a Vulkan/Adreno GPU build to follow — is the unpackaged, no-token on-device fallback.) After that: the image/audio backends (Image Playground / Stable Diffusion; Speech / Whisper / TTS), each in its own optional target like the zip backends. Context: [docs/ai-plugin.md](docs/ai-plugin.md).
