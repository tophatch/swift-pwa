...
On-device segmentation (`ai.vision.*`) reached full cross-platform parity in **v0.8.1** (above). The one remaining fast-follow — a **GPU execution-provider desktop build** for the CPU-only Linux/Windows ONNX Runtime backends — lands in **v0.8.2** behind a build-time `ai.onnx_gpu: true` opt-in: **Windows DirectML** (cross-vendor, all DX12 GPUs, in-box) and **Linux CUDA** (NVIDIA), auto-detected at runtime with transparent CPU fallback (`ai.vision.info` reports the active `provider`). There is no Vulkan-style one-artifact-all-vendors layer for ONNX Runtime as there is for llama.cpp — see [docs/proposals/onnx-gpu-execution-providers.md](docs/proposals/onnx-gpu-execution-providers.md) for the design and the research behind that constraint.
