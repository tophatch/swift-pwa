...
3. **On-device AI backends.** The `ai.*` plugin contract shipped (0.7) — `ai.info` / `ai.generate` / `ai.generateJSON` / streaming, plus multimodal **vision + audio input**, **text→image generation** (`ai.generateImage`), and **text→audio / TTS** (`ai.generateAudio`), schema-valid JSON on any backend — but no on-device backend is wired yet, so `ai.info` reports `available:false`. Next: the tier-1 text built-ins (Apple Foundation Models, Android Gemini Nano, Windows Phi Silica), the portable Gemma fallback (MLX / MediaPipe / ONNX / llama.cpp), and the image/audio backends (Image Playground / Stable Diffusion; Speech / Whisper / TTS), each in its own optional target like the zip backends. Context: [docs/ai-plugin.md](docs/ai-plugin.md).
