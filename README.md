...
4. **On-device segmentation (`ai.vision.*`) — in progress.** The contract (`SegmentationBackend`, `VisionPlugin`, `ai.vision.info` / `openSession` / `segment` / `closeSession`, reserved `segmentAll(Stream)` / `ensureModel` / `benchmark`) has landed in `SwiftPWACore`, and a real backend — **`MobileSAMBackend`** (Apple + Android, `SwiftPWASegmentation`) — runs MobileSAM's encoder plus one of two decoder variants as **ONNX Runtime** sessions, **verified end-to-end against real weights** (re-hosted at the `mobilesam-vendor` GitHub Release, sourced from Apache-2.0-licensed `Acly/MobileSAM`) — point/box/multi-point prompts against synthetic test images recovered ground-truth mask bounding boxes exactly. The packaging side (`Scripts/vendor-onnxruntime-apple.sh` / `-android.sh`, repackaging Microsoft's official artifacts, which don't import/link out of the box) is verified and published on both Apple and Android — Android confirmed **on an actual Galaxy Tab S10+** via `adb shell`. Android's image decode/resize (no CoreGraphics there) routes through a new `vision.preprocessImage` RPC to Kotlin's `BitmapFactory`. **A full on-device `openSession`/`segment` round trip is verified too** — on a Galaxy Z Fold7, against a real photo, correctly segmenting the prompted subject (IoU ~0.99). **`ai.local_onnx_runtime: true` in `pwa.json`** drives the whole build, same as `ai.local_llama` — on Android, `swift-pwa build` also resolves + checksum-verifies `libonnxruntime.so` per ABI and stages it into `jniLibs/` automatically (`OnnxRuntimeAndroidArtifact`; only `arm64-v8a` is published today). **`ai.vision.ensureModel`** rounds out the tier: a `MobileSAMBackend(cacheDirectory:)` downloads the three ONNX weight files on first use (resumable + checksum-pinned via the same `ModelDownloader` the llama GGUF uses, from the `mobilesam-vendor` release), so apps needn't bundle ~60 MB of weights — and on Android it writes straight to a real path, sidestepping the "an APK asset isn't a file ONNX Runtime can open" problem. `Examples/CritterFacts` uses this tier (verified end-to-end on macOS via a real download + on a Galaxy Z Fold7). Desktop backends (Linux/Windows) ship a `NoneBackend` in 0.8 (a documented parity exception, not an oversight); `segmentAll`/AMG, multi-point refine, and `ai.vision.benchmark` are fast-follow. Context: [docs/proposals/segmentation-plugin.md](docs/proposals/segmentation-plugin.md).
5. **Platform audio (capture / playback).** Native microphone capture and audio playback / device routing behind a plugin — lower-latency and more capable than the WebView's `getUserMedia` / `MediaRecorder`, which already cover the in-page cases (including feeding recordings to the `ai.*` audio input). Pairs with the on-device AI audio backends.
6. **Bidirectional bridge sessions (live streaming).** The bridge is request → server-stream-out today; a duplex/session primitive (client can push frames into an open subscription) would unlock real-time use cases — continuous-mic speech evaluation, collaborative streams — that the current `subscribe` can't express. A bridge-layer capability, broader than any one plugin.
7. **Delta updates** — ship binary diffs instead of full artifacts to cut update download size. Builds on (1); extend the publishing CLI to emit per-version patches and the runtime to apply them.
