...
3. **On-device AI backends.** The `ai.*` plugin contract shipped (0.7) — `ai.info` / `ai.generate` / `ai.generateJSON` / streaming, plus multimodal **vision + audio input**, **text→image generation** (`ai.generateImage`), and **text→audio / TTS** (`ai.generateAudio`), schema-valid JSON on any backend. The platform built-ins are now implemented on every OS that ships one — **Apple Foundation Models** (`SwiftPWAFoundationModels`) on Apple and **Android Gemini Nano** (`GeminiNanoBackend`, via ML Kit GenAI / AICore; opt in with `ai.gemini_nano` in `pwa.json`) on Android — alongside the portable **llama.cpp** backend (`SwiftPWALlama` — runs a GGUF model, GBNF-constrained JSON; opt in with `ai.local_llama`) on **Apple (Metal), Linux x86_64 (Vulkan GPU), and Windows x64 (Vulkan GPU)**. After that: **Windows Phi Silica** (the last tier-1 built-in) and the image/audio backends (Image Playground / Stable Diffusion; Speech / Whisper / TTS), each in its own optional target like the zip backends. Context: [docs/ai-plugin.md](docs/ai-plugin.md).
