...
struct AndroidBundler {
    ...
    func build() async throws -> URL {
        ...
        // base64-embedded into Core (generated by
        // Scripts/regenerate-bridge-js.sh from SwiftPWACore/Resources/bridge.js),
        // which is also what the runtime injects, so the two can't diverge.
        // Reading a SwiftPM resource bundle here would trap in a PREBUILT
        // single-file `swift-pwa` binary — the documented install path — which
        // is how the embed started; it now serves the runtime too.
