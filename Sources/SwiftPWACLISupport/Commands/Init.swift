...
enum Templates {
    ...
    static func releaseWorkflowYml(version: String) -> String {
        let cliBase = "https://github.com/tophatch/swift-pwa/releases/download/v\(version)"
        return """
        ...
        # `swift-pwa init`; tweak freely. The swift-pwa CLI is pinned to
        # v\(version) — bump the URLs below when you upgrade.
        name: Release
        on:
          push:
            tags: ["v*"]
        ...
        jobs:
          macos:
            ...
            steps:
              ...
              - name: Install the swift-pwa CLI
                run: |
                  curl -fsSL "\(cliBase)/swift-pwa-macos-arm64" -o /usr/local/bin/swift-pwa
          ...
          linux:
            ...
            steps:
              ...
              - name: Install the swift-pwa CLI
                run: |
                  curl -fsSL "\(cliBase)/swift-pwa-linux-x86_64" -o /usr/local/bin/swift-pwa
                  chmod +x /usr/local/bin/swift-pwa
          ...
          windows:
            ...
            steps:
              ...
              - name: Install the swift-pwa CLI
                shell: pwsh
                run: |
                  curl.exe -fsSL "\(cliBase)/swift-pwa-windows-x86_64.exe" -o swift-pwa.exe
