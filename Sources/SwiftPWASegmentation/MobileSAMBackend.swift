    ...
    /// A `SegmentationBackend` running MobileSAM (or full SAM, same decoder
    /// architecture) via two ONNX Runtime sessions — an encoder and a
    /// decoder — matching the reference SAM ONNX export contract (the same
    /// one `facebookresearch/segment-anything`'s own ONNX export script
    /// produces, and every MobileSAM ONNX port mirrors, since MobileSAM is
    /// a drop-in TinyViT encoder swap keeping SAM's decoder architecture):
    ///
    /// - **Encoder** — input `"input_image"` float32 `[1,3,1024,1024]`
    ///   (SAM's fixed resize-longest-side + top-left-pad + ImageNet-style
    ///   normalization, see `ImagePreprocessing`) → output
    ///   `"image_embeddings"` float32 `[1,256,64,64]`. **Verified against a
    ///   real exported graph** (`gifty-so/mobilesam-split-onnx` on Hugging
    ///   Face, encoder only — its weights are a placeholder/test upload, but
    ///   the graph's input/output names and shapes match exactly what's
    ///   coded here).
    /// - **Decoder** — inputs `"image_embeddings"` (the encoder's output),
    ///   `"point_coords"` float32 `[1,N,2]` (padded-1024-space, positive
    ///   points labeled `1`, negative `0`, a box's two corners labeled
    ///   `2`/`3`), `"point_labels"` float32 `[1,N]`, `"mask_input"` float32
    ///   `[1,1,256,256]` (zeros — no prior-mask refinement in v1),
    ///   `"has_mask_input"` float32 `[1]` (`0`), `"orig_im_size"` float32
    ///   `[2]` → outputs `"masks"` float32 `[1,numMasks,256,256]` (low-res
    ///   logits — this backend does its own upsample via
    ///   `MaskPostprocessing`, not relying on the graph to do it) and
    ///   `"iou_predictions"` float32 `[1,numMasks]`. **Not yet verified
    ///   against a real decoder graph** — the one on that same HF repo was
    ///   an empty/broken upload. This is the standard, widely-replicated
    ///   contract, but treat it as unconfirmed until run against real
    ///   weights (see `docs/proposals/segmentation-plugin.md`'s "Gaps to
    ///   close" section: model hosting is still an open decision).
    public actor MobileSAMBackend: SegmentationBackend {
        ...
        /// `encoderPath` / `decoderPath` are on-disk paths to the two ONNX
        /// graphs. No downloadable-model tier ships yet — see `ensureModel`
        /// below and the proposal doc's open model-hosting question — so
        /// callers currently provide their own local weights.
        /// `maxSessions` caps concurrently cached embeddings (each is
        /// `256*64*64*4` bytes ≈ 4 MB; the proposal recommends 2–3).
        public init(encoderPath: String, decoderPath: String, maxSessions: Int = 3) {
        ...
        public func openSession(_ request: OpenSessionRequest) async throws -> VisionSession {
            ...
            let outputs = try Self.mapping {
                try encoder.run(
                    inputs: ["input_image": .init(values: preprocessed.tensor, shape: [1, 3, 1024, 1024])],
        ...
        public func segment(_ request: SegmentRequest) async throws -> SegmentResult {
            ...
            for point in request.points ?? [] {
                let mapped = cached.preprocessed.mapPointToPadded(x: point.x, y: point.y)
            ...
            if let box = request.box, box.count == 4 {
                let topLeft = cached.preprocessed.mapPointToPadded(x: box[0], y: box[1])
                let bottomRight = cached.preprocessed.mapPointToPadded(x: box[2], y: box[3])
            ...
            let maskSize = Int(masksTensor.shape[2])
            var candidates: [VisionMask] = []
            for maskIndex in 0 ..< numMasks {
                let offset = maskIndex * maskSize * maskSize
                guard offset + maskSize * maskSize <= masksTensor.values.count else { continue }
                let logits = Array(masksTensor.values[offset ..< offset + maskSize * maskSize])
                let binaryMask = MaskPostprocessing.toSourceMask(
                    lowRes: logits, lowResSize: maskSize, preprocessed: cached.preprocessed, targetSize: 1024
                )
                guard let encoded = MaskPostprocessing.encodeRLE(
                    binaryMask, width: cached.preprocessed.originalWidth, height: cached.preprocessed.originalHeight
                ) else { continue }
            ...
            if !request.multimask, let best = candidates.first {
                return SegmentResult(masks: [best])
            }
