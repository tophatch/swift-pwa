    ...
    // NOTE: The body of this type is shared verbatim with the Linux copies
    // (Sources/SwiftPWAGTK/ + Sources/SwiftPWAGTK4/) and the Windows copy
    // (Sources/SwiftPWAWindows/) — only the `#if os(...)` guard differs. The
    // backends are separate Swift targets, so the apply logic can't live in one
    // file; keep all four copies in sync.
