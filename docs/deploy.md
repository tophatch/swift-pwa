...
| `--device <id>` | Which device. Android: an adb serial, or an `ip:port` for a wireless device (deploy runs `adb connect` first). iOS simulator: a simulator name or UDID. |
| `--simulator` | iOS: target a simulator (skips signing). Required for iOS until on-device deploy lands. |
...
On-device iOS deploy (a signed build installed via `xcrun devicectl`) is the next
cut. For now:

- **Simulator:** `swift-pwa deploy --target ios --simulator` builds, boots a
  simulator if none is running (an explicit `--device <name|udid>` is booted on
  demand; otherwise a booted one is reused, else the first available iOS
  simulator is booted), installs, and launches.
- **Device:** build a signed `.ipa` with `swift-pwa build --target ios --team
  <TEAMID> …` and install it (see [ios-setup.md](ios-setup.md)).
