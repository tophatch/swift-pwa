...
> **Status: root cause + fix proven in isolation; needs the re-host to ship.**
> An app that enables **both** `ai.local_llama` **and** `ai.local_onnx_runtime`
> (e.g. `Examples/CritterFacts` — on-device text *and* image) **cannot build for
> iOS**. Diagnosed to a hard xcodebuild limitation and validated the fix in a
> minimal package; shipping it is a vendor-script + re-host change.
