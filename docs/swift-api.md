...
downloadable-model tier yet). Android has no CoreGraphics/ImageIO, so image
decode/resize routes through a `vision.preprocessImage` RPC to Kotlin's
`BitmapFactory` instead. See
